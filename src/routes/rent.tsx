...
const search = z.object({ vehicle: z.string().optional() });

export const Route = createFileRoute("/rent")({
  validateSearch: search,
  head: () => ({
    meta: [
      { title: "Reserve a vehicle — Savana" },
      { name: "description", content: "Reserve a 4x4 safari vehicle: pick-up, drop-off, and dates." },
      { property: "og:title", content: "Reserve a vehicle — Savana" },
      { property: "og:description", content: "4x4 safari vehicles ready when you land." },
    ],
  }),
  component: RentWizard,
});

const STEPS = ["Vehicle", "Dates", "Pickup", "Driver"];

function RentWizard() {
  const { vehicle: initial } = Route.useSearch();
              ...
              {vehicles.map((opt) => (
                ...
                >
                  <div className="aspect-[16/10] overflow-hidden">
                    <img src={opt.image} alt="" loading="lazy" className="h-full w-full object-cover group-hover:scale-105 transition-smooth duration-700" />
                  </div>
                  <div className="p-4">

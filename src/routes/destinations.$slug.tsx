...
export const Route = createFileRoute("/destinations/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.d.name} — Savana Safaris` },
          { name: "description", content: loaderData.d.summary },
          { property: "og:title", content: `${loaderData.d.name} — Savana` },
          { property: "og:description", content: loaderData.d.summary },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen grid place-items-center px-4">
      <div className="text-center">
        ...
        <Button asChild variant="hero" className="mt-6"><Link to="/destinations">Browse all</Link></Button>
      </div>
    </div>
  ),
  ...
  return (
    <>
      <section className="relative h-[72svh] min-h-[520px] w-full overflow-hidden">
        <img src={d.image} alt={d.name} className="absolute inset-0 h-full w-full object-cover animate-ken-burns" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {d.gallery.map((src, i) => (
                <div key={i} className="aspect-square overflow-hidden rounded-2xl">
                  <img src={src} alt="" loading="lazy" className="h-full w-full object-cover hover:scale-110 transition-smooth duration-700" />
                </div>
              ))}
            </div>
          </div>
        ...
        <aside className="lg:sticky lg:top-28 h-fit">
          <div className="rounded-3xl bg-card shadow-elevated border border-border p-6">
            ...
            <Button asChild variant="hero" size="xl" className="w-full mt-6">
              <Link to="/book" search={{ destination: d.slug }}>Book this journey <ArrowRight className="h-5 w-5" /></Link>
            </Button>

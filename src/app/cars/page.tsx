...
export const metadata: Metadata = {
  ...
  description: "Rent a 4x4 Land Cruiser, safari van, premium SUV or open game-drive jeep across Africa.",
  openGraph: {
    title: "Safari Car Rental — Savana",
    description: "Self-drive or chauffeured vehicles built for the bush.",
  },
};
          ...
          {vehicles.map((v) => (
            <article key={v.slug} className="group rounded-3xl bg-card shadow-soft hover:shadow-elevated transition-smooth overflow-hidden border border-border">
              <div className="aspect-[16/10] overflow-hidden">
                <img src={v.image.src} alt={v.name} loading="lazy" className="h-full w-full object-cover group-hover:scale-105 transition-smooth duration-700" />
              </div>
              <div className="p-6 md:p-8">
                ...
                <Button asChild variant="forest" size="pill" className="mt-6">
                  <Link href={`/rent?vehicle=${v.slug}`}>Reserve this vehicle <ArrowRight className="h-4 w-4" /></Link>
                </Button>
              </div>
            </article>
          ))}
